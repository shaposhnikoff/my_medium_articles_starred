...
Unknown markup type 10 { type: [33m10[39m, start: [33m131[39m, end: [33m134[39m }
...
Unknown markup type 10 { type: [33m10[39m, start: [33m102[39m, end: [33m105[39m }
...
Unknown markup type 10 { type: [33m10[39m, start: [33m219[39m, end: [33m223[39m }
Unknown markup type 10 { type: [33m10[39m, start: [33m4[39m, end: [33m7[39m }
Unknown markup type 10 { type: [33m10[39m, start: [33m198[39m, end: [33m220[39m }
...
Suppose you have a [REST API](https://en.wikipedia.org/wiki/Representational_state_transfer) (e.g. GET /orders) and you want to restrict access to the authorized users only. 
...
**The HTTP protocol is *stateless***, that means a new request (e.g. GET /order/42) won’t know anything about the previous one, **so** **we need to reauthenticate for each new request** (fig.1).
...
In the IT world, the time goes fast and a solution that yesterday was commonly used might be outdated now. Server Side Sessions are one of those.
...
* *horizontal scaling – *scaling out your infrastructure is simpler and cost-effective as add a new server behind a [load balancer](https://en.wikipedia.org/wiki/Load_balancing_(computing));
...
In the initial scenario, behind the load balancer, there’s just one server. When a client will perform a request, using session id xyz, its record will be surely found in server’s memory (fig.3).
...
![Fig.4–A new server is behind the LB, it knows nothing about previous session so the user won’t be recognized.](https://cdn-images-1.medium.com/max/2050/1*aXM_BoiZQ-Sklrzq_SFi3A.png)*Fig.4–A new server is behind the LB, it knows nothing about previous session so the user won’t be recognized.*
...
A JWT token is essentially a long encoded text string. This string is composed of three smaller parts, separated by a dot sign. These parts are:
...
The following JSON explains what algorithm has been used to sign the token (alg) and what is the key (kid) that we need to use to validate it. One moment of patience, please, we will look into this soon. :)
...
The iss property is a [registered claim](https://tools.ietf.org/html/rfc7519#section-4.1), it represent the identity provider that issued the token — in this case [Amazon Cognito](https://aws.amazon.com/cognito/). Finally, we can add further claims based to our needs (e.g. admin claim).
...
Since token is self-contained, we own all the information needed for its validation. For example, we know that token has been signed using RS256 (alg property of the header) and a private key. Now we need to know **how to get the right public key to perform the validation**. Yes, the public key!
...
The iss property of the body represent the endpoint of the issuer (Amazon Cognito in our case, but it should be no great differences with other providers), copy that URI and prepend it to the string/.well-known/jwks.json. It should look something like:
...
That’s all for now. We have seen many things about JWT, but something else still misses:
